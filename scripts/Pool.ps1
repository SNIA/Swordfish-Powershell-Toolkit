...
process{
    $FullPoolCollection=@()
    $FullPoolCollectionOnly=@()
    switch ($PSCmdlet.ParameterSetName )
        {     'Default'           { $DefPoolCol     = @()
                                    $DefPoolColOnly = @()
                                    foreach ( $StorID in (Get-SwordfishStorage).id )
                                    {   $DefPoolCol        += Get-SwordfishPool -StorageID $StorID
                                        $DefPoolColOnly    += Get-SwordfishPool -StorageID $StorID -ReturnCollectionOnly $True
                                    }
                                    foreach ( $SSID in (Get-SwordfishStorageServices).id )
                                    {   $DefPoolCol        += Get-SwordfishPool -StorageServiceID $SSID
                                        $DefPoolColOnly    += Get-SwordfishPool -StorageServiceID $SSID -ReturnCollectionOnly $True
                                    }
                                    if ( $ReturnCollectionOnly )
                                    {   return ( $DefPoolColOnly | get-unique ) 
                                    } else 
                                    {   if ( $StoragePool ) 
                                        {   return ( $DefPoolCol | where-object { $_.id -eq $StoragePool} )
                                        } else 
                                        {   return $DefPoolCol
                                        }                                            
                                    } 
        ...
        {   $MemberSet = $PulledData.StoragePools
            foreach ( $PorPC in $Memberset )
            {   $PoolColOrPools = Invoke-RestMethod2 -uri ( $base + ($MemberSet.'@odata.id') )
                if ( -not $PoolColOrPools.Members ) 
                    {   $PoolMemberOrCollection = $MemberSet
                    } else 
                    {   $PoolMemberOrCollection = $PoolColOrPools.Members    
                    }
                foreach ( $MyPoolData in $PoolMemberOrCollection )
                {   $MyPool      = Invoke-RestMethod2 -uri ( $base + ($MyPoolData.'@odata.id') )
                    $FullPoolCollectionOnly  += $PoolColOrPools
                    $FullPoolCollection      += $MyPool
                }
            }
        if ( $ReturnCollectionOnly )
            ...
            {   IF ( $StoragePoolID )
                {   return ( $FullPoolCollection | where-object { $_.id -eq $StoragePoolId } )
                } else 
                {   return ( $FullPoolCollection )
                }
            }
        }
}
